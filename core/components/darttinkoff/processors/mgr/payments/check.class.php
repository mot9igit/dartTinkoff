...
class dartTinkoffPaymentCheckProcessor extends modObjectProcessor
{
	...
	public function process()
	{
		...
		if (!$dartTinkoff) {
			...
			foreach ($ids as $id) {
				...
				$response = $dartTinkoff->confirm($object->get('payment_id'));
				if($response['Success']){
					$object->set('status', $response['Status']);
					$object->set('updatedon', time());
					$object->save();
				}else{
					$dartTinkoff->to_log($response, 'ERROR');
				}
			}
			return $this->success();
		}
...
return 'dartTinkoffPaymentCheckProcessor';